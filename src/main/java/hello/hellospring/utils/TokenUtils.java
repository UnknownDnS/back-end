...
public final class TokenUtils{
    ...
    public static boolean isValidToken(String token) {
        try {
            ...
            log.error("Token is null"); return false; }
    }
    ...
    private static Map<String, Object> createHeader() {
        ...
        header.put("typ", "JWT"); header.put("alg", "HS256");
        header.put("regDate", System.currentTimeMillis());
        return header;
    }

    private static Map<String, Object> createClaims(UserVO user) { // 공개 클레임에 사용자의 이름과 이메일을 설정하여 정보를 조회할 수 있다.
    ...
    private static UserRole getRoleFromToken(String token) {
        Claims claims = getClaimsFormToken(token);
        return (UserRole) claims.get("userRole");
    }
}
